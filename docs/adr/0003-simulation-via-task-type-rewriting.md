...
**A task's declared output `type_hint` picks the editor for its simulated value, and nothing else.** The author supplies the value. Simulation does not check that value against the hint, and does not pass the hint on: the rewrite sends only `{name, value}`, and how the value is encoded on the wire follows from the value itself. The hint means the same thing here as everywhere else — a note for whoever reads the blueprint, not a rule. Simulation reads it from the blueprint instead of asking for it a second time, so the editor always matches what the author wrote.

Two different fields are in play here. A task's `type` is `{agent_type}.{tool_name}`, and it is what simulation rewrites. An input, output or param has a `type_hint` (once called `type`), which describes a value and is never rewritten.

**`type_hint` stays free-form; the editor degrades.** Three tiers:

1. **Native form** — `str`, `int`, `float`, `bool`, `timestamp` render as text / number / checkbox / date inputs. This covers 52 of the 59 `type_hint` declarations across the current example blueprints.
...
- **Simulation is the first feature to read `outputs[].type_hint` at all, so wrong declarations become visible — but they break nothing.** Nothing else reads it: `outputs_to_keys` drops it before dispatch (`TaskAssignmentMessage.output_keys` is `list[str]`), `_enrich_data_with_types` works out a type from the value instead, and binding checks the tool's Python annotation, not the blueprint. Declarations are therefore already wrong in places — `system.start` declares `timestamp` while `SystemAgent.start_process` returns `time.time()`, a float. Simulating an existing blueprint shows these up as an odd-looking editor, not as a failure. Fixing the declaration is optional; the JSON textarea always works.
- **A wrong hint gives a simulated value of the wrong type.** The editor decides the type of what the author enters, so `timestamp` — the most common hint in the example blueprints — gives a date-picker string where the real agent returns a float. Simulation gets scheduling and dataflow right; it does not promise a simulated value has the type the real one would. Where that matters, fix the declaration or use the JSON textarea.
- **The editor has to accept both spellings.** Blueprint JSON takes either `type_hint` or the old `type`, and always writes back `type_hint`, so blueprints saved before the rename still carry `type`. Simulation reads `type_hint` and falls back to `type`. The frontend schema and types still say `type`, and need updating before the editor can rely on `type_hint` alone.
- A closed enum for `type_hint` was rejected: it would invalidate existing blueprints and would need extending every time an agent returns a new type, in a system whose extension story is that anyone may write an agent in any language. Graceful degradation to a JSON textarea costs nothing and never blocks an author.
- The `antikythera-agents describe` / `tools.json` catalogue specified in `ARCHITECTURE.md` now exists (backend ADR-0002): each tool descriptor lists a `type_hint` per input, param and output, taken from the tool's Python annotations, and binding checks those annotations rather than the blueprint. It says what a task type really produces, so it is the eventual fix for the mismatch above — the descriptor describes the tool, the blueprint describes what the author believed. Connecting simulation to it is out of scope for v1, and would be a convenience rather than a fix: the blueprint's own declaration picks a good enough editor, and nothing in a run depends on either hint being right.
